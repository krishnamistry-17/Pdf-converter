    ...
    <div className=" relative my-2">
      <button
        className="flex items-center gap-2 text-left font-medium
         bg-white text-text-heading px-4 py-2 rounded-xl shadow-sm border border-gray-100"
        onClick={() => toggleOpen()}
      >
        <FaLanguage />
      ...
      {open && (
        <div className="absolute top-12 left-0 z-50 bg-white text-text-heading shadow-sm border border-gray-100 rounded-xl p-4">
          <div className="grid grid-cols-2 gap-2">
            {languages.map((lang) => (
              <button
                key={lang.code}
                className="text-sm text-text-heading font-medium hover:bg-primary/10 p-2 rounded-md"
